...
<div :style='{"width":"1200px","padding":"20px","margin":"10px auto","position":"relative","background":"#f5f5f5"}'>
    <el-form
      ...
      label-width="80px"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"transparent"}' label="课程编号" prop="kechengbianhao">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"transparent"}' label="报名须知" prop="baomingxuzhi">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="报名须知"
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"transparent"}' label="详情" prop="xiangqing">
            <editor 
  ...
  export default {
    ...
    methods: {
      ...
      init(type) {
        ...
        this.$http.get('option/kaojijibie/kaojijibie', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('kaojixinxi/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
                  ...
                  params: params
                     ...
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('kaojixinxi/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
